...
import { useParams, useNavigate, useLocation } from '@/lib/navigation-compat';
import { 
    ...
    >
      ...
      <div style={{ 
        ...
        <div className="flex flex-wrap gap-3 sm:gap-4 mt-3 sm:mt-4">
          ...
          {(endpoint as { rateLimit?: string }).rateLimit && (
            <span style={{ fontSize: 'clamp(0.65rem, 2.5vw, 0.75rem)', color: theme.colors.textSecondary }}>
              Rate limit: <strong style={{ color: theme.colors.text }}>{(endpoint as { rateLimit?: string }).rateLimit}</strong>
            </span>
          )}
          {endpoint.scope && (
            <span style={{ fontSize: 'clamp(0.65rem, 2.5vw, 0.75rem)', color: theme.colors.textSecondary }}>
