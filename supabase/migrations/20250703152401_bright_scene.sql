...
RETURNS trigger AS $$
BEGIN
  -- Update positions for all pending entries
...
CREATE TRIGGER trigger_calculate_position
  ...
  FOR EACH STATEMENT EXECUTE FUNCTION calculate_waitlist_position();
...
-- Allow authenticated users to read all waitlist entries (for admin)
CREATE POLICY "Authenticated users can read all waitlist entries" ON waitlist_entries
  FOR SELECT TO authenticated
  USING (true);

-- Allow authenticated users to update waitlist entries (for admin)
