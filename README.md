...
## SEO and Serverless Functions

This project leverages Netlify Lambda (serverless) functions to dynamically generate `robots.txt` and `sitemap.xml`. This approach provides greater flexibility and control over SEO configurations.

-   **`robots.txt`:** Generated by `netlify/functions/robots-txt.js`.
-   **`sitemap.xml`:** Generated by `netlify/functions/sitemap-xml.js`.

These functions ensure that the latest content and routing information is always reflected, overriding any static files.
